...
def export_stl(base_path, scale):
    ...
    export_stl(base_path + '.stl', scale)
...
def convert_osm(osm_obj_path, osm_blend_path, osm_stl_path):
    scale = 1000
    ...
    export_stl_file(osm_stl_path, scale)

def main(argv):
    if len(argv) != 3:
        print("usage: convert.py [path to input .obj file] [path to output .blend file] [path to output .stl file]")
        return
    
    convert_osm(argv[0], argv[1], argv[2])
